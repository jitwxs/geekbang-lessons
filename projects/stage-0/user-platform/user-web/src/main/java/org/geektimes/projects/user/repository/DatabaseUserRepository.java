...
public class DatabaseUserRepository implements UserRepository {
    ...
    private Connection getConnection() {
        final DBConnectionManager dbConnectionManager = ComponentContext.getInstance().getComponent("bean/DBConnectionManager");
        return dbConnectionManager.getConnection();
    }

    @Override
    public boolean save(User user) {
        return executeUpdate(INSERT_USER_DML_SQL, COMMON_EXCEPTION_HANDLER, user.getId(), user.getName(), user.getPassword(), user.getEmail(), user.getPhoneNumber()) != 0;
    }
    ...
    private User convertUser(ResultSet resultSet) throws Exception {
    ...
    protected int executeUpdate(String sql, Consumer<Throwable> exceptionHandler, Object... args) {
        ...
        try {
            ...
            for (int i = 0; i < args.length; i++) {
                Object arg = args[i];
                Class<?> argType = arg.getClass(), wrapperType = wrapperToPrimitive(argType);
